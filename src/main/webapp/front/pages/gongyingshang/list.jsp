    ...
    <div class="container py-3">
        ...
        <div class="row">
            ...
            <div class="col-lg-4 event-right wthree-event-right">
                ...
                <input type="text" v-model="searchForm.gongyingshangName" placeholder="供应商名称" autocomplete="off"
                       class="search-input">     
                <input type="text" v-model="searchForm.lianxirenName" placeholder="联系人姓名" autocomplete="off"
                       class="search-input">                 
                <select class="search-input" v-model="searchForm.gongyingshangXinyongdengjiTypes">
                    <option value="" selected>请选择信用等级</option>
                    <option v-for="(item,index) in gongyingshangXinyongdengjiTypesList" v-bind:key="index"
                            :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}
                    </option>
                </select>                            <div style="display: flex;justify-content: space-evenly;align-items: center;">
                        <div id="btn-search" class="button-submit"><i class="layui-icon layui-icon-search"></i>搜索</div>
                        <div v-if="isAuth('gongyingshang','新增')"  @click="jump('../gongyingshang/add.jsp')" class="button-submit"><i class="layui-icon">&#xe654;</i>添加</div>
                    </div>
                </form>
            </div>
        </div>
