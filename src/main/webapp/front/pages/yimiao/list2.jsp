...
<div style=" width:20%; ">
    ...
    <ul id="category" style="padding-top:100px;">
        <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../yimiao/list.jsp'?'bbbb':''" style="width: 100%;">
            <a :href="'javascript:jump(\''+item.url+'\')'"
               :class="item.url=='../yimiao/list.jsp'?'aaaa':''">{{item.name}}</a>
        </li>
    </ul>
</div>        <!-- 个人中心菜单 -->
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.yimiaoTypes=="" || searchForm.yimiaoTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in yimiaoTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.yimiaoTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../yimiao/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加疫苗信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>{{item.yimiaoUuidNumber}}</td>
                        <td><img :src="baseUrl+item.yimiaoPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.yimiaoAddress}}</td>
                        <td>{{item.yimiaoKucunNumber}}</td>
                        <td>{{item.yimiaoNewMoney}}</td>
                        <td>{{item.yimiaoValue}}</td>
                        <td><button v-if="item.yimiaoContent!=null&&item.yimiaoContent!=''" @click="showContentFunction(item.yimiaoContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../yimiao/add.jsp?yimiaoId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.yimiaoTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`yimiao/update`, 'post', {
                    id:id,
//                    yimiaoTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`yimiao/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/yimiao/list2.jsp")

        // var id = http.getParam('id');

        //疫苗类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.yimiaoTypes = $(this).attr('index');
            pageList();
        });
           ...
           function yimiaoTypesSelect() {
            ...
            function pageList() {
                // 获取列表数据
                http.request('yimiao/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('yimiao/list', 'get', vue.searchForm, function (res1) {
