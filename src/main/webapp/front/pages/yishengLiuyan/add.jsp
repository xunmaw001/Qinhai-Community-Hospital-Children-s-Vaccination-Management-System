...
<body style="background: #fff; ">
            ...
            <form class="layui-form layui-form-pane add" lay-filter="myForm">
                ...
                <!--<div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="yishengId" id="yishengId" lay-filter="yishengId">
                            <option v-for="(item,index) in yishengList" v-bind:key="index" :value="item.id" :selected="detail.yishengId == item.id?true:false" :key="item.id">{{ item.yishengName }}</option>
                        </select>
                    </div>
                </div>-->
                <!-- 级联表 用户 -->
    ...
    <!-- 当前表的 -->
                <!-- 级联表的表id -->
                <input type="hidden" :value="detail.yishengId" id="yishengId" name="yishengId"/>
                <!-- 级联表的表id -->
            ...
            <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                ...
                <div class="layui-input-block">
                    <textarea class="main_borderColor layui-input" style="width: 100%;height: 100px;border-radius: 5px;padding: 10px" v-model="detail.yishengLiuyanText" lay-verify="required" :readonly="ro.yishengLiuyanText" name="yishengLiuyanText" id="yishengLiuyanText" autocomplete="off"></textarea>
                </div>
            </div>
                    <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/yishengLiuyan/list.jsp")

            //var yishengId = http.getParam('yishengId');
            let yishengId = localStorage.getItem("yishengId")
            if(yishengId != null && yishengId != ""){
                //单个 医生查询
                initializationYisheng(yishengId);
            }
            //列表 医生查询
            initializationYishengList();
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('yishengLiuyanId')){
                http.request(`yishengLiuyan/info/${http.getParam('yishengLiuyanId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                data = data.field;
                if(http.getParam('yishengLiuyanId')){
                    data["id"]= http.getParam('yishengLiuyanId')
                }
                // 提交数据
                http.requestJson(`yishengLiuyan/${http.getParam("yishengLiuyanId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../yishengLiuyan/list.jsp");
                                http.remove('yishengLiuyanId')
                            // back();
                        });
                    }else{
        ...
        function initializationYisheng(id){
            layui.http.request("yisheng/detail/" + id, 'get', {}, function (res) {
                if(res.code ==0){
                    vue.yisheng = res.data;
                    vue.detail.yishengId=vue.yisheng.id//赋值给detail
                }
            });
        }
        //列表 医生查询
        function initializationYishengList(){
            layui.http.request("yisheng/page?page=1&limit=100&sort=&order=", 'get', {}, function (res) {
                if(res.code ==0){
