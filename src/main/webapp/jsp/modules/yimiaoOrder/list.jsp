...
<!--/Pre Loader -->
        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">疫苗预约管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">疫苗预约列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">疫苗预约列表</h3>
                            <div class="table-responsive mb-3" id="tableDiv">
                                <div class="col-sm-12">
                                                                         
                                        <label>订单编号</label>
                                        <input type="text" id="yimiaoOrderUuidNumberSearch" placeholder="请输入订单编号" aria-controls="tableId" class="form-control form-control-sm">
                                                                                                                                                                                                                         
                                        <label>订单类型</label>
                                        <select name="yimiaoOrderTypesSelectSearch" id="yimiaoOrderTypesSelectSearch" aria-controls="tableId" class="form-control form-control-sm"></select>
                                        ...
                                        <span class="导入导出">
                                            导入<input name="file" id="yimiaoOrderExcelFile" type="file" class="btn btn-success" style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success" onclick="download('/upload/yimiaoOrderMuBan.xls')">导入模板下载</button>
                                        </span>
                                ...
                                <table id="tableId" class="table table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <th>疫苗名称<i id="yimiaoNameIcon"></i></th>
                                        <th>疫苗照片<i id="yimiaoPhotoIcon"></i></th>
                                        <th>疫苗类型<i id="yimiaoTypesIcon"></i></th>
                                        <th>用户姓名<i id="yonghuNameIcon"></i></th>
                                        <th onclick="sort('yimiao_order_uuid_number')">订单编号<i id="yimiaoOrderUuidNumberIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('buy_number')">人数<i id="buyNumberIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('yimiao_order_true_price')">实付价格<i id="yimiaoOrderTruePriceIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('yimiao_order_time')">预约时间<i id="yimiaoOrderTimeIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('yimiao_order_types')">订单类型<i id="yimiaoOrderTypesIcon" class="fa fa-sort"></i></th>
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //订单编号
            var yimiaoOrderUuidNumberSearchInput = $('#yimiaoOrderUuidNumberSearch');
            if( yimiaoOrderUuidNumberSearchInput != null){
                if (yimiaoOrderUuidNumberSearchInput.val() != null && yimiaoOrderUuidNumberSearchInput.val() != '') {
                    searchForm.yimiaoOrderUuidNumber = $('#yimiaoOrderUuidNumberSearch').val();
                }
            }
                         
                //订单类型
            var yimiaoOrderTypesSelectSearchInput = document.getElementById("yimiaoOrderTypesSelectSearch");
            if(yimiaoOrderTypesSelectSearchInput != null){
                var yimiaoOrderTypesIndex = yimiaoOrderTypesSelectSearchInput.selectedIndex;
                if( yimiaoOrderTypesIndex  != 0){
                    searchForm.yimiaoOrderTypes= document.getElementById("yimiaoOrderTypesSelectSearch").options[yimiaoOrderTypesIndex].value;
                }
            }
                    getDataList();
        }
        ...
        function getDataList() {
            http("yimiaoOrder/page", "GET", {
                ...
                yimiaoOrderDelete: 1,
                //本表的
                yimiaoOrderUuidNumber: searchForm.yimiaoOrderUuidNumber,
                yimiaoOrderTypes: searchForm.yimiaoOrderTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //订单编号
            var yimiaoOrderUuidNumberCell = document.createElement('td');
            yimiaoOrderUuidNumberCell.innerHTML = item.yimiaoOrderUuidNumber;
            row.appendChild(yimiaoOrderUuidNumberCell);
            ...
            //实付价格
            var yimiaoOrderTruePriceCell = document.createElement('td');
            yimiaoOrderTruePriceCell.innerHTML = item.yimiaoOrderTruePrice;
            row.appendChild(yimiaoOrderTruePriceCell);


            //预约时间
            var yimiaoOrderTimeCell = document.createElement('td');
            yimiaoOrderTimeCell.innerHTML = item.yimiaoOrderTime;
            row.appendChild(yimiaoOrderTimeCell);


            //订单类型
            var yimiaoOrderTypesCell = document.createElement('td');
            yimiaoOrderTypesCell.innerHTML = item.yimiaoOrderValue;
            row.appendChild(yimiaoOrderTypesCell);
            ...
            if(window.sessionStorage.getItem('role') == "用户" && item.yimiaoOrderTypes == 101){
                //取消
            ...
            if(
                ...
                deliverBtn.innerHTML = "同意";
                btnGroup.appendChild(deliverBtn);
            }
            if(window.sessionStorage.getItem('role') == "用户" && item.yimiaoOrderTypes == 103){
                //接种
        ...
        function upload() {
            /*导入数据*/
            $('#yimiaoOrderExcelFile').fileupload({
                url: baseUrl + 'file/upload',
                headers: {token: window.sessionStorage.getItem("token")},
                dataType: "json",
                done: function (e, data) {
                    http("yimiaoOrder/batchInsert?fileName="+data.result.file, "get", {} , (res) => {
                        if(res.code == 0){
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("yimiaoOrder/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
        ...
        function wuyong(id) {
            ...
            if (mymessage == true) {
                httpJson("yimiaoOrder/update", "POST", {
                    id:id,
                    yimiaoOrderTypes:2
                }, (res) => {
                    if(res.code == 0){
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addyimiaoOrder", "addyimiaoOrder");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function yimiaoOrderTypesSelectSearch() {
            var yimiaoOrderTypesSelectSearch = document.getElementById('yimiaoOrderTypesSelectSearch');
            if(yimiaoOrderTypesSelectSearch != null) {
                yimiaoOrderTypesSelectSearch.add(new Option('-请选择-',''));
                if (yimiaoOrderTypesOptions != null && yimiaoOrderTypesOptions.length > 0){
                    for (var i = 0; i < yimiaoOrderTypesOptions.length; i++) {
                            yimiaoOrderTypesSelectSearch.add(new Option(yimiaoOrderTypesOptions[i].indexName,yimiaoOrderTypesOptions[i].codeIndex));
                    }
                }
            }
        }
...
// 101 102 103 104 105
        ...
        function refund(id) {
            ...
            http("yimiaoOrder/refund?id="+id, "GET",{}, (res) => {
                if(res.code == 0){
                    alert('操作成功');
                    getDataList();
                }else{
        ...
        // 同意
        function deliver(id) {
            var mymessage = confirm("确定要同意吗？");
            if(!mymessage){
                alert("已取消操作");
                return false;
            }
            http("yimiaoOrder/deliver?id="+id, "GET",{}, (res) => {
                if(res.code == 0){
                    alert('操作成功');
                    getDataList();
                }else{
        ...
        function receiving(id) {
            var mymessage = confirm("确定要接种吗？");
            if(!mymessage){
                alert("已取消操作");
                return false;
            }
            http("yimiaoOrder/receiving?id="+id, "GET",{}, (res) => {
                if(res.code == 0){
                    alert('操作成功');
                    getDataList();
                }else{
